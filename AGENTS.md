...
- Orders and order items are the local purchase record. Order creation transactionally copies cart price snapshots, converts the active cart, and starts with `pending` order/payment statuses; payment status, Stripe session IDs, and paid timestamps are owned by the upcoming payment workflow.
