...
- Orders and order items are the local purchase record. Order creation transactionally copies cart price snapshots, converts the active cart, and starts with `pending` order/payment statuses. The Payment module owns payment status, Stripe session IDs, and paid timestamps.
- Stripe checkout requires `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET`. Checkout success/cancel URLs default to the first `CORS_ORIGINS` origin (`/payment/success` and `/cart`) and can be overridden with the corresponding `STRIPE_CHECKOUT_*_URL` variables. Stripe webhook verification depends on Nest's raw request body and must remain unauthenticated; only a valid Stripe signature may update payment or order state.
- Cards are unique product-linked instances. Registration atomically claims an `available` card for the current user; cards are never physically deleted, and archive/block operations preserve ownership data.
- Map markers contain precise player locations. Public marker reads return only active, unexpired markers within a maximum 25 km radius; expire markers quickly and move to PostGIS before location-query scale requires it.
